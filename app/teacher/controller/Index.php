    ...
    public function tongbu()
    {
        ...
        foreach ($tclist as $key => $value) {
            ...
            if($temp)
            {
                ...
                $temp->teacher_id = $value->teacher_id;
                ...
                if($temp->this_time < $value->this_time)
                {
                    ...
                    $temp->ip = $value->ip;
                    $temp->lasttime = $value->lasttime;
                    $temp->thistime = $value->thistime;
                }
        ...
        return '已经将教师信息同步到管理员表中：1、教师信息与管理员信息已经关联：使用教师信息替换管理员信息；2、教师与管理员信息未关联：在管理员表中创建教师信息。帐号为手机号，密码为原密码。';
    }
}
