...
class Banji extends AdminBase
{
    ...
    // 设置班主任
    public function banzhuren($id)
    {
        $list['data']['id'] = $id;

        $bj = new bjmod;
        $bjInfo = $bj->where('id', $id)
                ->field('id, school_id, banzhuren')
                ->with([
        ...
        $list['set'] = array(
            ...
            ,'butname'=>'设置'
            ,'formpost'=>'PUT'
            ,'url'=>'/teach/banji/updatebanzhuren/' . $id,
        );
    ...
    // 更新班主任
    public function updatebanzhuren($id)
    {
        // 获取参数
        $src = $this->request
            ->only([
        ...
        // 验证表单数据
        $validate = new \app\teach\validate\Banji;
        $result = $validate->scene('banzhuren')->check($src);
        $msg = $validate->getError();
        if(!$result){
            return json(['msg'=>$msg,'val'=>0]);;
        }
        ...
        // 更新数据
        $bjInfo = bjmod::find($id);
        $bjInfo->banzhuren = $src['banzhuren'];
        $data = $bjInfo->save();
