...
layui.extend({
    ...
    var obj = {
        ...
        // 上传txt
        uploadTxt: function(uploadId, category, serurl, backId) {
            upload.render({
                elem: '#' + uploadId, //绑定元素
                url: '/tools/file/upload', //上传接口
                    ...
                    if (res.val == 1) {
                        $('#' + backId).val(res.url);
                        $.post("/tools/readtxt/read", { url: res.url }, function(data) {
                            $('#xinghao').val(data.xinghao)
                            $('#xuliehao').val(data.xuliehao)
                            $('#mac').val(data.mac)
                            $('#biaoqian_time').val(data.date)
                            $('#gailan').html(data.gailan)
                            $('#info').val(data.gailan)
                            $('#infos').val(data.info)
                            $('#xitong').val(data.xitong)
                            $('#xitong_time').val(data.xitong_time)
                        });
                    }
                    layer.msg(res.msg);
                },
